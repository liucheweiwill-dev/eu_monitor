...
- **三站合成一條 Google 查詢、只開一個分頁**（2026-09-23 實測）：
  `(site:eeas.europa.eu (…)) OR (site:consilium.europa.eu (…)) OR (site:nato.int (…))`
  回「約有 0 項結果」——同一天 EEAS 單獨查是約 38 筆。不是被截斷，是**整條靜默失效**，
  頁面上沒有任何「查詢太長」之類的提示。這比漏抓更糟：使用者看到 0 會以為三站都沒新東西。
  **三個分頁是必要的，不是還沒優化。**
- **`&num=100` 讓一頁顯示全部結果**（2026-09-23 實測）：Google 已經不接受，
  照樣 10 筆一頁、照樣有「下一頁」。結果超過 10 筆就一定要翻頁——
  人自己看時會翻，但任何「自動收集結果網址」的做法都要處理分頁，
  否則第 2 頁以後會靜默消失（eu_verify 的書籤 v1 就中過這個，v2 已修）。
